...
export function activate(context: vscode.ExtensionContext) {
  ...
  const prompt_suggestion = vscode.commands.registerCommand(
    "aider.generateAutocomplete",
    async () => {
      ...
      const action = await vscode.window.showQuickPick(
        ["Generate Autocomplete", "Debug"],
        {
          placeHolder: "Choose an action",
        }
      );
      if (action === "Generate Autocomplete") {
        const res = await fetch("http://0.0.0.0:8000/code/completion", {
          method: "POST",
          headers: {
        ...
        if (
          ...
          // Write into the user field some code
          editor.edit((editBuilder) => {
            editBuilder.insert(selection.end, response["completion"] as string);
          });
          vscode.window.showInformationMessage(
            `Generated completion in ${time}s`
          );
  ...
  context.subscriptions.push(prompt_suggestion);
}
